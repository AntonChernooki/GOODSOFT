...
export class RepairRequestService {
  ...
  private readonly apiUrl = `${environment.apiUrl}/repair-requests`;

  getAll(): Observable<RepairRequestResponseDto[]> {
    return this.http.get<RepairRequestResponseDto[]>(this.apiUrl);
  }

  getById(id: number): Observable<RepairRequestResponseDto> {
    return this.http.get<RepairRequestResponseDto>(`${this.apiUrl}/${id}`);
  }

  create(repairRequestCreateDto: RepairRequestCreateDto): Observable<RepairRequestResponseDto> {
    return this.http.post<RepairRequestResponseDto>(this.apiUrl, repairRequestCreateDto);
  }

  update(id: number, repairRequestUpdateDto: RepairRequestUpdateDto): Observable<RepairRequestResponseDto> {
    return this.http.patch<RepairRequestResponseDto>(`${this.apiUrl}/${id}`, repairRequestUpdateDto);
  }
  ...
  updateStatus(id: number, status: RepairRequestStatus): Observable<RepairRequestResponseDto> {
    return this.http.patch<RepairRequestResponseDto>(`${this.apiUrl}/${id}/status`, { status });
  }

  getByStatus(status: RepairRequestStatus): Observable<RepairRequestResponseDto[]> {
    return this.http.get<RepairRequestResponseDto[]>(`${this.apiUrl}/status/${status}`);
  }

  getByDriverId(driverId: number): Observable<RepairRequestResponseDto[]> {
    return this.http.get<RepairRequestResponseDto[]>(`${this.apiUrl}/driver/${driverId}`);
  }

  getByCarId(carId: number): Observable<RepairRequestResponseDto[]> {
    return this.http.get<RepairRequestResponseDto[]>(`${this.apiUrl}/car/${carId}`);
  }
}
